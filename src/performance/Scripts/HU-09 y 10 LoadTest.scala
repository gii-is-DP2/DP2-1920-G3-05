...
class HU09LoadTest extends Simulation {
	...
	object LoginNewUser {
		val loginNewUser = exec(http("LoginNewUser")
			.get("/login")
			.headers(headers_0)
			.resources(http("request_2")
			.get("/login")
	...
	setUp(
		positiveScn.inject(rampUsers(500) during (100 seconds)),
		negativeScnUnsubscribe.inject(rampUsers(500) during (100 seconds)),
		inscribeBadMeetingScenario.inject(rampUsers(500) during (100 seconds))
